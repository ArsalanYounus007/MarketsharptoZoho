...
public class MarketSharpClient {
   ...
   testProductType(service);
    ...
    private static void testContact(MarketSharpCrmApiService service)
    {
        ...
        {
            try {
				AddRow.addRow("ID",contact.getId());
				AddRow.addRow("First Name", contact.getFirstName());
				AddRow.addRow("Business name", contact.getBusinessName());
				AddRow.addRow("Email Adress", contact.getEmail1());
				AddRow.addRow("Phone No:",contact.getContactPhone().toString());
				AddRow.addRow("Address", contact.getPrimaryAddressId());
				AddRow.addRow("Source", contact.getSource());
				AddRow.addRow("Last Update", contact.getLastUpdate().toString());
				
				customFields = contact.getCustomField();
	            for (CustomField customField : customFields)
	            {   AddRow.addRow("Name", customField.getName());
    ...
    private static void testProductType(MarketSharpCrmApiService service) throws Exception
    {
        ...
        for (ProductType productType : productTypeQuery)
        {
            ...
            AddRow.addRow("Product ID", productType.getId());
            AddRow.addRow("ID:" , productType.getId());
            AddRow.addRow("ProductNAME:" , productType.getName());
    ...
    private static void testInquirySourcePrimary(MarketSharpCrmApiService service)
    {
        ...
        {
        	try {
				AddRow.addRow("Inquiry_source_ID",inquirySource.getId());
				AddRow.addRow("First Name", inquirySource.getName());
				AddRow.addRow("City",inquirySource.getCompany().getAddressCity());
				AddRow.addRow("Email", inquirySource.getCompany().getEmail());
    ...
    private static void testAppointmentQuery2(MarketSharpCrmApiService service) throws Exception
    {
        ...
        for (Appointment appointment : appointmentsQuery)
        {
            ...
            //PRINT OUT RESULTS
            try {
				AddRow.addRow("Appointment ID",appointment.getId().toString());
				AddRow.addRow("Appointment Date", appointment.getSetDate().toString());
				AddRow.addRow("Salesperson1", appointment.getSalesperson1().getName());
				AddRow.addRow("Subject", appointment.getSubject());
				AddRow.addRow("LastUpdate", appointment.getLastUpdate().toString());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
            ...
            if (inquiry != null) {
                AddRow.addRow("-->INQDT:",inquiry.getInquiryDate().toString());
                AddRow.addRow("-->NOTE:" , inquiry.getNote().toString());
            }
        }
